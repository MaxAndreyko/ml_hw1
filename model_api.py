...
POLY_PATH = './poly.pkl'
PATHS = [ENCODER1_PATH, ENCODER2_PATH, SCALER2_PATH, POLY_PATH, MODEL_PATH]
...
def make_prediction(df, enc1_path, enc2_path, scaler2_path, poly_path, model_path):
    df = pd.DataFrame(df)
    try:
    ...
    df_only_num = df_cat_models[num_features]
    ...
    df_only_num = df_scaled[num_features]
    df_cat_models = df_scaled.drop(num_features,
                                   axis=1)
...
def predict_items(file: UploadFile = File(...)):
    ...
    output_file.seek(0)
    reponse = StreamingResponse(output_file, media_type="text/csv",
